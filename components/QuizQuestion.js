export default class QuizQuestion {
    ...
    _getTemplate(reference, templateSelector, containerSelector) {
        return reference.querySelector(templateSelector)
            .content.querySelector(containerSelector).cloneNode(true);
    }
    ...
    getQuestionElement() {
        const questionElement = this._getTemplate(document, this._selector.quizQuestionTemplate, this._selector.quizQuestionContainer);
        const questionItem = this._getRandomQuestion();
        questionElement.querySelector(this._selector.quizQuestionText).textContent = questionItem.question;
        questionElement.querySelector(this._selector.quizQuestionText).id = this._id;

        const answerList = questionElement.querySelector(this._selector.quizAnswerList);
        const answerTemplate = this._getTemplate(questionElement, this._selector.quizAnswerTemplate, this._selector.quizAnswerContainer);
